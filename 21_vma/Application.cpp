...
void Application::initVulkan()
{
    ...
    createMultisampledColorResources();
...
void Application::createPipeline()
{
    ...
    piplineInfo.pDepthStencilState = &depthStencil;
...
void Application::drawFrame()
{
    ...
    uint32_t imageIndex = swapChain.currentImage(currentFrame);
    if (imageIndex == -1)
    {
        beforeCleanupSwapchain();
...
)
{
    ...
    imageInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE; // graphics queue family only
...
VkImageView Application::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels)
{
    ...
    {
        throw std::runtime_error("failed to create taxture image view");
    }
