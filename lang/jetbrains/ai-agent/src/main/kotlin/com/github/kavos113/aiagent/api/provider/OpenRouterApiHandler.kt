...
class OpenRouterApiHandler(options: ApiHandlerOptions.OpenRouterHandlerOptions): ApiHandler {
    ...
    override fun sendRequest(param: ApiParam): ApiResponse {
        val model = when(param.model) {
            is ApiModel.AnthropicModel -> throw IllegalArgumentException("Anthropic model not supported for OpenRouter API")
            is ApiModel.OpenAiModel -> throw IllegalArgumentException("OpenAI model not supported for OpenRouter API")
            is ApiModel.OpenRouterModel -> param.model.model
        }
