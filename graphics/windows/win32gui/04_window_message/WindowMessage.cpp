...
static std::unordered_map<UINT, std::string> messageMap = {
    { WM_NULL ,"WM_NULL" },
    { WM_CREATE ,"WM_CREATE" },
    ...
    { WM_APP ,"WM_APP" },
    { WM_USER ,"WM_USER" },
    { WM_INTERCEPTED_WINDOW_ACTION ,"WM_INTERCEPTED_WINDOW_ACTION" },
    { WM_TOOLTIPDISMISS ,"WM_TOOLTIPDISMISS" },
    { WM_CLOAKED_STATE_CHANGED ,"WM_CLOAKED_STATE_CHANGED" },
};

std::string getWindowMessageName(UINT uMsg)
{
    if (messageMap.find(uMsg) != messageMap.end())
    {
        return messageMap[uMsg];
    }
    else
    {
        std::stringstream ss;
        ss << "unknown: " << std::hex << uMsg;
        return ss.str();
    }
}
